...
ifeq ($(TARGET_VENDOR_SHOW_MAINTENANCE_VERSION),true)
    ...
    ifeq ($(SHOW_PIPELINE_ID_AS_VERSION_MAINTENANCE),true)
        CM_VERSION_MAINTENANCE := $(CI_PIPELINE_ID)
    else
...
ifeq ($(CM_BUILDTYPE), RELEASE)
    ...
    ifeq ($(CM_VERSION_MAINTENANCE),0)
        ifeq ($(LINEAGE_VERSION_APPEND_TIME_OF_DAY), TRUE)
            LINEAGE_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d_%H%M%S)$(CI_PIPELINE_ID)-$(CM_BUILDTYPE)$(CM_EXTRAVERSION)-$(CM_BUILD)
        else
            LINEAGE_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d)$(CI_PIPELINE_ID)-$(CM_BUILDTYPE)$(CM_EXTRAVERSION)-$(CM_BUILD)
        endif
    else
        ifeq ($(LINEAGE_VERSION_APPEND_TIME_OF_DAY), TRUE)
...
ifneq ($(PRODUCT_DEFAULT_DEV_CERTIFICATE),build/target/product/security/testkey)
    ifneq ($(CM_BUILDTYPE), UNOFFICIAL)
        ...
        ifeq ($(CM_VERSION_MAINTENANCE),0)
            CM_DISPLAY_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(TARGET_VENDOR_RELEASE_BUILD_ID)$(CI_PIPELINE_ID)
        else
